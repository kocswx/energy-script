...
def shift_prod_noz_order(cur, db, param):
    print('rpt_shift_prod_noz_order:',
          cur.execute(
              "DELETE FROM %s.rpt_shift_prod_noz_order WHERE GROUP_ID='%s' and STATION_ID='%s' AND SHIFT_NO='%s'"
...
def shift_emp_prod_pay(cur, db, param):
    ...
    sql = "INSERT INTO %s.`rpt_shift_emp_prod_pay` (`GROUP_ID`, `STATION_ID`, `SHIFT_NO`, `SHIFT_DATE`, `PROD_ID`, `PROD_NAME`, `EMP_ID`, `EMP_NAME`, `PAY_TYPE_ID`, `PAY_TYPE_NAME`, `CNT`, `VOL`, `RECE_AMT`, `REAL_AMT`, `DISC_AMT`) " \
          ...
          "FROM %s.fuel_order WHERE GROUP_ID='%s' and STATION_ID='%s' AND PAY_TYPE_ID<>2 and SHIFT_NO='%s' GROUP BY EMP_ID,EMP_NAME,PROD_ID,PROD_NAME,PAY_TYPE_ID,PAY_TYPE_NAME, GROUP_ID,STATION_ID,SHIFT_NO" % (
              db.DB_REPORT, param.shift_date, db.DB_REPORT, param.group_id, param.station_id, param.shift_no)
    print(sql)
    cur.execute(sql)
